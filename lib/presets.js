...
export const REALTIME_PRESETS = [
    {
        ...
        wssUrl: '',
        defaultModel: '',
        // 不连云端 → 无 keyPreset（与 wssUrl/defaultModel 的「不可用」标记一致）。
        keyPreset: '',
...
export function realtimePresetById(id) {
...
export function autoModeForModel(model) {
    return CHAT_MODE_MODEL_RE.test(model) ? 'chat' : 'transcriptions';
}
/** 解析最终调用通道：显式模式优先，auto 按模型名判定。 */
export function resolveAsrMode(mode, model) {
    if (mode === 'chat' || mode === 'transcriptions')
        return mode;

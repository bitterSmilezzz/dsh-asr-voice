 ...
 *  （路由传 502 = 真正的上游/服务端故障）。 */
export function statusOfBodyError(error, fallback) {
...
export async function readUpstreamJson(res, maxBytes = MAX_UPSTREAM_JSON_BYTES) {
    ...
    if (overflow)
        throw new Error(`upstream response too large (> ${maxBytes} bytes)`);
...
export function redactSecret(text, maxLen = 200) {
    const redacted = text
        // 认证头回显（最典型的泄漏形状）
...
export function isTrusted(req) {
    ...
    const stripDefaultPort = (h) => h.replace(/:(?:80|443)$/, '');
    let hostName = '';
    let hostWithPort = '';
    try {
...
export function guardRoute(req, methods = ['POST']) {
    if (!isTrusted(req))
        return { ok: false, status: 403, payload: { ok: false, reason: 'forbidden: host/origin not trusted' } };
    if (!methods.includes(req.method ?? ''))
        return { ok: false, status: 405, payload: { ok: false, reason: 'method not allowed' } };
    return null;
}

...
`;
		...
		*/
		...
		async function readKeyState(p) {
			...
			try {
				...
				const view = response.result.ok === true ? response.result.value.credentials?.[ref] : void 0;
				return {
					ref,
		...
		*/
		async function saveKey(p, value) {
			...
			try {
				const failure = outcomeError((key === "" ? await credentialsApi.unset({ ref }) : await credentialsApi.set({
					ref,
					value: key
				})).result);
		...
		*/
		...
		function createEnergyVad(sampleRate, tuning, events) {
		...
		*/
		function VoiceChatButton(props) {
			...
			const failByCode = (code) => {
				const msg = code === "no-mic" || code === "mic-denied" || code === "silent-device" || code === "no-audio-context" ? t("errNoMic") : code === "network" ? t("errWebSpeechNetwork") : code === "provider-unreachable" ? t("errSegmentedUnreachable") : code === "no-worklet" || code === "capture-failed" ? t("errSegmentedUnsupported") : t("errNoSpeechSupport");

		...
		*/
		function VoiceChatButton(props) {
			...
			const failByCode = (code) => {
				const msg = code === "no-mic" || code === "mic-denied" ? t("errNoMic") : code === "network" ? t("errWebSpeechNetwork") : t("errNoSpeechSupport");
			...
			const begin = () => {
				...
				engineRef.current = createBrowserRealtime(tuning.language, {
					settleMs: tuning.settleMs,
					tailMs: tuning.tailMs
				}, {
					onPartial: (text) => {

...
- 输入框工具行**麦克风按钮**（`conversation.input.right`）：点击开始/结束（默认手动关麦，点停止即整段去识别；可选静音自动停止），可选按住说话
...
- **录音为静音（提示「未检测到声音」）时，优先检查浏览器授权弹窗/地址栏站点设置里选中的输入设备是否为内置麦克风**——远程控制/直播软件安装的虚拟音频设备常被默认选中，导致浏览器录到纯静音；插件静音守卫会拦截此类录音并提示，不会把静音发给云端识别。

...
- **barge-in 门控已在真 Chromium 管线验过（2026-09-01，qa-asr loop 探针）**：真
  AudioWorklet 采集 + 真引擎，假麦克风 WAV 电平时序场景全绿——纯回声（低电平
  循环）零误触、零上行；回声后接高电平人声恰好触发一次打断，触发后段恢复转写。
  **这轮抓出一个门控缺陷**：TTS 有声-静音交替时窗口 p25 被静音空隙帧拉低，TTS
  自身音量反超「背景」误断——修复为「低于基线（vad.rms）的帧不参与背景学习 +
  背景未学到不判 over」，补 2 例回归单测。**仍未覆盖**：真实扬声器→空气→麦克风
  的声学回环（假设备采集不进 AEC 参考，消除率与泄漏系数需真机复测）。

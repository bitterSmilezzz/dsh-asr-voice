 ...
 *
 * API key 全部存 host settings（服务端），浏览器只经私有 JSON 路由调用。
 * 纯 Node HTTP（全局 fetch / FormData），无平台专属二进制 → macOS / Windows 双平台。
 */
...
import { registerOptimizeRoute, type LlmOptimizeConfig } from './optimize.ts';
...
/** Host context slice this plugin consumes (webServer comes from the type merge). */
...
export function apply(ctx: AsrVoiceHostContext): void {
  ...
  ctx.effect(() => registerOptimizeRoute((def) => ctx.webServer.register(def), getLlmConfig), 'asr-voice: optimize route');
}

...
import { realtimePresetById, REALTIME_PRESETS } from './presets.ts';
...
export function providerView(
  ...
  idFallback: string,
  modeOverride?: string,
): { id: string; preset: string; name: string; baseUrl: string; apiKey: string; model: string; mode: string } {
  return {
...
function createRealtimeProvider(
  ...
  if (preset === undefined) {
    ...
    return {
      connect: async () => {
        throw new Error(
          `unknown realtime provider "${pid}" — expected '' / 'builtin' or one of: ${REALTIME_PRESETS.map((p) => p.id).join(', ')}`,
        )
      },
    }
  }

...
export const FAKE_REALTIME_DEFAULTS: FakeRealtimeTuning = {
  rms: 0.02,
  silenceMs: 600,
  minSpeechMs: 200,
  maxSegmentMs: 8_000,
...
/** int16 小端字节块 → RMS（0~1）。 */
function rmsInt16(data: Uint8Array): number {
...
class FakeRealtimeConnection implements RealtimeProviderConnection {
  onEvent: ((ev: RealtimeProviderEvent) => void) | null = null

  private readonly silenceWindows: number
  private readonly minSpeechWindows: number
  private readonly maxSegmentWindows: number
  private readonly partialWindows: number

  /** 攒窗缓冲（一个分析窗的 int16 字节）。 */
  private readonly win = new Uint8Array((SAMPLE_RATE * WINDOW_MS) / 1000 * 2)
  private winLen = 0
  ...
  constructor(private readonly tuning: FakeRealtimeTuning) {
  ...
  private emit(ev: RealtimeProviderEvent): void {
    if (!this.closed) this.onEvent?.(ev)
  }

  /** 处理一个完整分析窗（int16 字节，长度 = 窗采样数 × 2）。 */
  private handleWindow(bytes: Uint8Array): void {
...
export function createFakeRealtimeProvider(
  tuning: FakeRealtimeTuning = FAKE_REALTIME_DEFAULTS,
): RealtimeProvider {
  return {
    connect: async (): Promise<RealtimeProviderConnection> => new FakeRealtimeConnection(tuning),
  }
}
